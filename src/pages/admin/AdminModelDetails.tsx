...
export default function AdminModelDetails() {
  ...
  async function translateSingle(jmId: number, entityName: string) {
    ...
    setTransProgress({ open: true, entityType: "model_detail", entityName, jmId, logs: [], errors: [], done: 0, total: targetLocales.filter((l: string) => l !== "zh-CN").length });
    try {
      const { data, error } = await supabase.functions.invoke("db-translate", {
        body: { action: "translate_single", entityType: "model_detail", jmId: String(jmId) },
      });
  ...
  async function batchTranslate() {
    ...
    const selectedRows = visibleModelDetails.filter(d => selectedIds.includes(d.id));
    ...
    const batchAbortRef = batchCancelRef;
    setTransProgress({
      open: true,
      entityType: "model_detail",
      entityName: `批量翻译 ${selectedRows.length} 个车型`,
      jmId: 0,
      logs: [],
      errors: [],
      done: 0,
    ...
    for (const detail of selectedRows) {
      if (batchAbortRef.current) break;
      setTransProgress(prev => ({
        ...prev,
        entityName: `车型 ${completed + 1}/${selectedRows.length}: ${detail.name}`,
        done: completed,
      ...
      try {
        const { data, error } = await supabase.functions.invoke("db-translate", {
          body: { action: "translate_single", entityType: "model_detail", jmId: String(detail.jm_id) },
        });
        if (error) {
          allErrors.push({ locale: "-", error: `${detail.name}: ${error.message}` });
        } else if (data?.details?.length) {
          for (const d of data.details) {
            if (d.error) allErrors.push({ locale: d.locale, error: `${detail.name}: ${d.error}` });
            else allLogs.push({ locale: d.locale, key: d.key, source: d.source, translated: d.translated });
          }
        }
      } catch (e: any) {
        allErrors.push({ locale: "-", error: `${detail.name}: ${e?.message || "失败"}` });
      }
      completed++;
    }
    ...
    setTransProgress(prev => ({
      ...prev,
      entityName: batchAbortRef.current ? `已中断 · 完成 ${completed}/${selectedRows.length}` : `完成 ${completed} 个车型`,
      done: completed,
      logs: allLogs.slice(-50),
      errors: allErrors,
    }));
        ...
        <div className="mt-6">
          ...
          <div className="flex items-center gap-2 mb-3">
            <button
              ...
              onClick={async () => {
                const allRows = visibleModelDetails.filter(d => d.jm_id > 0);
                if (allRows.length === 0) return;
                setBatchTranslating(true);
                batchCancelRef.current = false;
                const batchAbortRef = batchCancelRef;
                setTransProgress({ open: true, entityType: "model_detail", entityName: `批量翻译 ${allRows.length} 个车型`, jmId: 0, logs: [], errors: [], done: 0, total: allRows.length });
                let allLogs: TranslationProgress["logs"] = [];
                let allErrors: TranslationProgress["errors"] = [];
                let completed = 0;
                ...
                async function worker() {
                  while (queue.length > 0) {
                    ...
                    try {
                      const { data, error } = await supabase.functions.invoke("db-translate", { body: { action: "translate_single", entityType: "model_detail", jmId: String(detail.jm_id) } });
                      if (error) allErrors.push({ locale: "-", error: `${detail.name}: ${error.message}` });
                      else if (data?.details?.length) {
                        for (const d of data.details) {
                          if (d.error) allErrors.push({ locale: d.locale, error: `${detail.name}: ${d.error}` });
                          else allLogs.push({ locale: d.locale, key: d.key, source: d.source, translated: d.translated });
                    ...
                    } catch (e: any) { allErrors.push({ locale: "-", error: `${detail.name}: ${e?.message || "失败"}` }); }
                    completed++;
                    setTransProgress(prev => ({ ...prev, entityName: `车型 ${completed}/${allRows.length}`, done: completed, logs: allLogs.slice(-100), errors: allErrors }));
                  }
                }

                await Promise.all(Array.from({ length: Math.min(CONCURRENCY, allRows.length) }, () => worker()));
                setTransProgress(prev => ({ ...prev, entityName: batchAbortRef.current ? `已中断 · 完成 ${completed}/${allRows.length}` : `完成 ${completed} 个车型`, done: completed, logs: allLogs.slice(-100), errors: allErrors }));
                setBatchTranslating(false);
                clearEntityTranslationCache();
                await loadDbModelDetails();
              }}
            ...
            >
              {batchTranslating ? "翻译中..." : `一键翻译全部车型（${visibleModelDetails.filter(d => d.jm_id > 0).length}）`}
            </button>
            {batchTranslating ? (
              <button type="button" onClick={() => { batchCancelRef.current = true; }} className="inline-flex items-center gap-1 rounded-lg bg-red-100 px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-200 transition-colors">
                停止
              </button>
            ) : null}
                  ...
                  visibleModelDetails.map((detail) => {
                    ...
                    <tr key={detail.id} className={`hover:bg-zinc-50 ${deleted ? 'opacity-60' : ''}`}>
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        <button
                          type="button"
                          onClick={() => translateSingle(detail.jm_id, detail.name)}
                          disabled={translatingId === detail.jm_id}
                          className="text-sm font-medium text-blue-600 hover:text-blue-800 disabled:text-blue-300 mr-3"
                        >
                          {translatingId === detail.jm_id ? "翻译中..." : "翻译"}
                        </button>
                        <button
                          type="button"
                          onClick={() => {
