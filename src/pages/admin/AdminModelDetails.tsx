...
export default function AdminModelDetails() {
  ...
  // Entity translation
  const { translating: transDetails, progress: transProgress, error: transError, setError: setTransError, translateEntities } = useEntityTranslation("model_detail");
      ...
      {activeTab === 'db-view' && (
        <div className="mt-6">
          ...
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            ...
            <button
              type="button"
              disabled={transDetails || transTargetLocales.length === 0}
              onClick={async () => {
                const ids = visibleModelDetails.map(d => d.jm_id).filter(id => id > 0);
