...
export default function DeleteResourceModal(props: {
  intent: DeleteIntent;
  confirmText: string;
  busy: boolean;
  ...
  return (
    <Modal
      open={intent.open}
      title={intent.open ? `确认删除资源：${intent.brandName} / ${intent.seriesName}` : "确认删除资源"}
      onClose={() => {
        if (busy) return;
        onClose();
      }}
        ...
        <div className="p-6">
          ...
          {intent.scope === "images" ? (
            <div className="mb-4">
              ...
              <select
                value={intent.imageDeleteMode}
                onChange={(e) => onChangeImageDeleteMode(e.target.value as any)}
                className={inputCls()}
              >
              ...
              <div className="mt-2 text-xs text-zinc-500">
                图片展示以 car_pictures 为主，缺失时回退 model_image_config。
              </div>
            </div>
          ) : null}
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              ...
              onClick={onConfirm}
              disabled={busy || !ok}
              className={smallButtonCls("danger") + " px-4 py-2"}
            >
              {busy ? "删除中..." : "确认删除"}
            </button>
          </div>
