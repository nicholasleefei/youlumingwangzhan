...
export default function ModelExterior() {
  ...
  // @ts-ignore
  const exteriorImages = model.exterior_images?.filter((img: string) => img.trim()) || [];
    ...
    <div className="min-h-screen bg-primary-dark text-text-primary relative overflow-hidden">
      ...
      <main className="mx-auto max-w-7xl px-4 py-32 relative z-10">
        ...
        <div className="mb-8 flex items-center gap-2 text-sm">
          ...
          <span className="text-text-tertiary">/</span>
          <span className="text-text-primary">{t("model.exterior", "外观")}</span>
        </div>
        ...
        <div className="mb-10">
          <h2 className="text-2xl font-bold text-text-primary mb-6 px-1">{t("model.exterior", "外观")}</h2>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {exteriorImages.map((imageUrl, index) => (
                ...
                >
                  <img
                    src={imageUrl}
                    alt={`${name} ${t("model.exterior")} ${index + 1}`}
